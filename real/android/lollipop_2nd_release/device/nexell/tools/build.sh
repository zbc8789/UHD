...
function parse_args()
{
    TEMP=`getopt -o "b:r:t:i:d:a:k:chfvs" -- "$@"`
    eval set -- "$TEMP"
...
function build_kernel()
{
    if [ ${BUILD_ALL} == "true" ] || [ ${BUILD_KERNEL} == "true" ]; then
		...
		if [ ${BUILD_FIRST} == "true" ];then
			...
			local arch=$(get_arch)
			cp arch/arm/configs/${kernel_config} .config
			cp -a ${TOP}/device/nexell/s5p4418_realv1c/boot/*.o ${TOP}/kernel/drivers/media/video/nvp/
		fi
       
     	
        yes "" | make  oldconfig
        make uImage -j8 modules
...
function post_process()
{
    if [ ${BUILD_DIST} == "false" ]; then
		...
		cp ${TOP}/device/nexell/${BOARD_NAME}/boot/2ndboot.bin ${RESULT_DIR}
